...
// Test importing payment protocol
const testCode = `
...
async def handle_chat(ctx: Context, sender: str, msg: ChatMessage):
    ...
    if PAYMENT_AVAILABLE:
        response = "Payment protocol is available!"
    else:
        response = f"Payment protocol import failed: {PAYMENT_ERROR}"
...
async function main() {
  ...
  console.log('Uploading payment import test...');
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (s.compiled) {
      console.log('\n✅ Agent compiled!');
      console.log('Now message the agent to see if payment protocol imported.');
      return;
    }
  }
  console.log('\n❌ Still not compiled - payment import likely causes the issue');
}
