...
describe('tokenize()', () => {
    it('sends POST to /api/agents/tokenize with agentAddress', async () => {
        ...
        const result = await tokenize({ agentAddress: 'agent1qf8test', name: 'GiftBot', symbol: 'GIFT' }, client);
        restore();
        assert.ok(capturedUrl.includes('/api/agents/tokenize'), 'URL should include /api/agents/tokenize');
        assert.deepEqual(capturedBody.agentAddress, 'agent1qf8test');
    ...
    it('propagates AgentLaunchError from the client on 401', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse({ message: 'Unauthorized' }, 401)));
        const client = makeClient();
        await assert.rejects(() => tokenize({ agentAddress: 'agent1qtest' }, client), (err) => {
    ...
    it('returns the status as pending_deployment for a new token record', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse(mockTokenizeResponse)));
        const client = makeClient();
...
describe('getToken()', () => {
    it('calls GET /api/agents/token/:address with the correct address', async () => {
        ...
        restore();
        assert.ok(capturedUrl.includes(`/api/agents/token/${TOKEN_ADDRESS}`), `URL should contain the address. Got: ${capturedUrl}`);
    ...
    it('throws AgentLaunchError when address is not found (404)', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse({ message: 'Token not found' }, 404)));
        const client = makeClient();
        await assert.rejects(() => getToken('0xDEADBEEF', client), (err) => {
            assert.ok(err instanceof AgentLaunchError);
            assert.equal(err.status, 404);
            assert.equal(err.serverMessage, 'Token not found');
            return true;
        });
        restore();
    });
    it('returns price and progress fields from the API response', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse(mockToken)));
        const client = makeClient();
...
describe('listTokens()', () => {
    ...
    it('returns token list and total from the API response', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse({ tokens: [mockToken], total: 1 })));
        const client = makeClient();
    ...
    it('returns empty tokens array when no tokens exist', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse({ tokens: [], total: 0 })));
        const client = makeClient();
        const result = await listTokens({}, client);
        restore();
