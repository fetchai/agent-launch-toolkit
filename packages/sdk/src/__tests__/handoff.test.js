...
describe('generateDeployLink()', () => {
    ...
    it('throws when tokenId is 0', () => {
        assert.throws(() => generateDeployLink(0), /positive integer/i);
    });
    it('throws when tokenId is negative', () => {
        assert.throws(() => generateDeployLink(-5), /positive integer/i);
    });
    it('throws when tokenId is a float', () => {
        assert.throws(() => generateDeployLink(1.5), /positive integer/i);
    });
});
// ---------------------------------------------------------------------------
// generateTradeLink
...
describe('generateTradeLink()', () => {
    ...
    it('appends amount as a string value', () => {
        ...
        assert.equal(link, `${DEFAULT_BASE}/trade/${TOKEN_ADDRESS}?action=sell&amount=500.5`);
    });
...
describe('generateBuyLink()', () => {
    ...
    it('includes amount when provided', () => {
        ...
        assert.equal(link, `${DEFAULT_BASE}/trade/${TOKEN_ADDRESS}?action=buy&amount=250`);
    });
