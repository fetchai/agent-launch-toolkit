...
import { generateTradeLink, generateTradeLinkFromOptions, getTokenPrice, } from '../market.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
const DEFAULT_BASE = 'https://agent-launch.ai';
const mockToken = {
    id: 1,
    name: 'GiftBot',
    symbol: 'GIFT',
...
describe('market.generateTradeLink() — positional API', () => {
    ...
    it('uses AGENT_LAUNCH_BASE_URL env var when no client is provided', () => {
        ...
        assert.ok(link.startsWith('https://my-custom-env.com'), `Expected env var base URL. Got: ${link}`);
        if (saved !== undefined) {
            process.env['AGENT_LAUNCH_BASE_URL'] = saved;
        }
        else {
...
describe('market.generateTradeLinkFromOptions()', () => {
    ...
    it('passes action=sell when specified in options', () => {
        const client = new AgentLaunchClient({ baseUrl: DEFAULT_BASE });
        const link = generateTradeLinkFromOptions(TOKEN_ADDRESS, { action: 'sell', amount: 200 }, client);
...
describe('getTokenPrice()', () => {
    ...
    it('propagates fetch errors as AgentLaunchError', async () => {
        const restore = installFetchMock(() => Promise.resolve(makeResponse({ message: 'Not found' }, 404)));
        const client = new AgentLaunchClient({
            baseUrl: 'https://test.local',
        });
        const { AgentLaunchError } = await import('../types.js');
        await assert.rejects(() => getTokenPrice('0xBAD', client), (err) => {
