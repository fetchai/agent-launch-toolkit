...
describe('AgentLaunchClient.get — error handling', () => {
  it('throws AgentLaunchError when the response status is 404', async () => {
    const restore = installFetchMock(() =>
      Promise.resolve(makeResponse({ message: 'Not found' }, 404, 'Not Found')),
    );

    const client = new AgentLaunchClient({ baseUrl: 'https://test.local' });

    await assert.rejects(
      () => client.get('/api/agents/token/0xbad'),
      (err: unknown) => {
  ...
  it('includes the server error message in the thrown error', async () => {
    const restore = installFetchMock(() =>
      Promise.resolve(
        makeResponse({ message: 'Token not found' }, 404, 'Not Found'),
      ),
    );
    ...
    assert.ok(
      caughtError.message.includes('Token not found'),
      'message should embed serverMessage',
    );
  });

  it('throws AgentLaunchError on 500 Internal Server Error', async () => {
    const restore = installFetchMock(() =>
      Promise.resolve(makeResponse({ error: 'Internal error' }, 500, 'Internal Server Error')),
    );

    const client = new AgentLaunchClient({ baseUrl: 'https://test.local' });

    await assert.rejects(
      () => client.get('/api/agents/tokens'),
      (err: unknown) => {
        assert.ok(err instanceof AgentLaunchError);
        assert.equal((err as AgentLaunchError).status, 500);
        return true;
      },
    );
...
describe('AgentLaunchClient.post — authentication', () => {
  it('throws AgentLaunchError with status 0 when apiKey is absent on POST', async () => {
    ...
    await assert.rejects(
      () => client.post('/api/agents/tokenize', { agentAddress: 'agent1q...' }),
      (err: unknown) => {
        assert.ok(err instanceof AgentLaunchError, 'should be AgentLaunchError');
        assert.equal((err as AgentLaunchError).status, 0);
        assert.ok(
          (err as AgentLaunchError).message.includes('API key'),
  ...
  it('attaches X-API-Key header on authenticated POST', async () => {
    ...
    const restore = installFetchMock((_url, init) => {
      ...
      return Promise.resolve(
        makeResponse({ success: true, data: { token_id: 1, handoff_link: 'https://agent-launch.ai/deploy/1', name: 'TestToken', symbol: 'TEST', description: 'test', image: '', status: 'pending_deployment' } }),
      );
    });
  ...
  it('throws AgentLaunchError on non-2xx POST response', async () => {
    const restore = installFetchMock(() =>
      Promise.resolve(makeResponse({ message: 'Unauthorized' }, 401, 'Unauthorized')),
    );
    ...
    await assert.rejects(
      () => client.post('/api/agents/tokenize', {}),
      (err: unknown) => {
        assert.ok(err instanceof AgentLaunchError);
