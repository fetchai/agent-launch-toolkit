/**
 * Integration tests for Swarm-starter template generation
 *
...
// ---------------------------------------------------------------------------
// Swarm-starter template generation
// ---------------------------------------------------------------------------

describe('Integration: Swarm-starter template generation', () => {
  it('generates valid Python from swarm-starter template', () => {
  ...
  it('strict mode rejects missing required variables', () => {
    assert.throws(
      () => generateFromTemplate('swarm-starter', {}, { variables: {}, strict: true }),
      /agent_name/i,
...
describe('Integration: Preset -> Swarm-starter round-trip', () => {
  const presetNames = [
    'writer',
    'social',
    'community',
  ...
  for (const presetName of presetNames) {
    it(`preset "${presetName}" generates complete project with zero unresolved placeholders`, () => {
      ...
      // Verify config references swarm-starter template
      const config = JSON.parse(result.agentlaunchConfig);
      assert.equal(
        config.template,
        'swarm-starter',
        `${presetName}: config should reference swarm-starter template`,
      );
    });
  }
});
