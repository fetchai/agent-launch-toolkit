...
// ---------------------------------------------------------------------------
async function avGet(apiKey, path) {
...
async function avPost(apiKey, path, body, method = "POST") {
    const url = `${AGENTVERSE_API}${path}`;
    const response = await fetch(url, {
        method,
        headers: {
    ...
    if (!response.ok) {
        ...
        throw new Error(`${method} ${url} failed with status ${response.status} ${response.statusText}${detail}`);
    }
...
export async function deployToAgentverse(args) {
    ...
    const created = await avPost(args.apiKey, "/hosting/agents", { name: agentName });
    ...
    await avPost(args.apiKey, `/hosting/agents/${agentAddress}/code`, { code: JSON.stringify(codeArray) }, "PUT");
    ...
    if (args.secrets && Object.keys(args.secrets).length > 0) {
        for (const [name, secret] of Object.entries(args.secrets)) {
            await avPost(args.apiKey, "/hosting/secrets", {
    ...
    // Step 4: Start agent
    await avPost(args.apiKey, `/hosting/agents/${agentAddress}/start`, {});
    // Step 5: Poll for running status (up to ~60 s, checking every 5 s)
    let status = "starting";
    for (let i = 0; i < 12; i++) {
        await new Promise((resolve) => setTimeout(resolve, 5000));
        try {
            const agentStatus = await avGet(args.apiKey, `/hosting/agents/${agentAddress}`);
