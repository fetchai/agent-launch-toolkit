...
const AGENT_TYPE_CONFIGS: Record<AgentType, AgentTypeConfig> = {
  faucet: {
    ...
    businessLogicComment:
      "Check balance, validate request, send FET via wallet integration",
  },
  research: {
    domain: "research",
    ...
    businessLogicComment:
      "Query external APIs, summarise findings with LLM, return structured data",
  },
  trading: {
    domain: "trading",
    description:
      "Execute autonomous trading strategies and report positions",
    freeRequestsPerDay: 20,
    rateLimitPerMinute: 30,
    businessLogicComment:
      "Analyse market data, evaluate strategy signals, report recommended actions",
  },
  data: {
    domain: "data",
    ...
    businessLogicComment:
      "Fetch from data sources, normalise schema, cache results, serve to callers",
  },
};
...
function generateAgentPy(name: string, config: AgentTypeConfig): string {
...
function generateReadme(name: string, type: AgentType, config: AgentTypeConfig): string {
  return `# ${name}
...
\`\`\`bash
curl -X POST ${SCAFFOLD_API_URL}/agents/tokenize \\
...
# Optional: AgentLaunch API base URL (default: dev Cloud Run URL; set AGENT_LAUNCH_ENV=production for prod)
AGENTLAUNCH_API=${SCAFFOLD_API_URL}
...
export async function scaffoldAgent(args: {
  ...
  const agentType: AgentType = args.type ?? "research";

  if (!(agentType in AGENT_TYPE_CONFIGS)) {
    throw new Error(
      `Unknown agent type "${agentType}". Valid types: faucet, research, trading, data`,
    );
  }
