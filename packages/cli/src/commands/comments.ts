...
export function registerCommentsCommand(program: Command): void {
  program
    ...
    .description(
      "List or post comments for a token by its contract address",
    )
      ...
      async (
          ...
          if (message.length > 500) {
            if (isJson) {
              console.log(
                JSON.stringify({
                  error:
                    "--post message must be 500 characters or fewer",
                }),
              );
            } else {
              console.error(
        ...
        try {
          ...
          if (Array.isArray(response)) {
            ...
            comments =
              (response as { data?: Comment[]; comments?: Comment[] })
                .data ??
              (response as { data?: Comment[]; comments?: Comment[] })
                .comments ??
              [];
        ...
        if (isJson) {
          console.log(
            JSON.stringify({ comments, total: comments.length }),
          );
          return;
        }
        ...
        console.log("─".repeat(60));
        console.log(
          `${comments.length} comment(s). Use --post "<message>" to add one.\n`,
        );
      },
    );
}
