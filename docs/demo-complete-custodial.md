...
Trade tokens without managing private keys. Two wallet types:

- **User wallet** (default): derived from your user identity — stable forever, no matter how many agents you create
- **Agent wallet**: derived from a specific agent address — for autonomous per-agent trading

Pass `agentAddress` to use an agent's wallet. Omit it to use your personal user wallet.
...
```
┌─────────────────────────────────────────────────────────────┐
│  Call POST /agents/buy with X-API-Key header               │
│  Optional: include agentAddress in body for agent wallet   │
└─────────────────────────┬───────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  WE VALIDATE your key against Agentverse                   │
│  We get your user ID (stable identity)                     │
└─────────────────────────┬───────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
│                                                             │
│  No agentAddress → hash("user:42") → 0x94bC...  (yours)   │
│  agentAddress    → hash("agent1q...") → 0x5FE... (agent)  │
│                                                             │
│  Same identity = Same wallet. Always. Forever.             │
└─────────────────────────┬───────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
**Key insight**: We don't store private keys. We derive them on-demand using HD wallet derivation — from your user ID (personal wallet) or agent address (agent wallet). Stateless. Deterministic. Secure.
